...
export type Selector = { kind: 'lastBrowser' } | { kind: 'foreground' } | { kind: 'alias'; alias: string };
...
/**
 * Two snapshots are "too close to call" when their observation times fall inside this window.
...
function ageMs(s: Snapshot, now: number): number {
  return now - Date.parse(s.observedAt);
}
...
export function resolve(
  ...
  const sorted = [...pool].sort((a, b) => Date.parse(b.observedAt) - Date.parse(a.observedAt));
  const newest = sorted[0];
  const newestAt = Date.parse(newest.observedAt);

  const contenders = sorted.filter((s) => newestAt - Date.parse(s.observedAt) <= AMBIGUITY_WINDOW_MS);
